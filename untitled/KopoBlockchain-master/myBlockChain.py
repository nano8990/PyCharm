...
def writeBlockchain(blockchain):
    ...
    for block in blockchain:
        ...
        blockchainList.append(blockList) # 리스트 안에 리스트를 넣음(이중 배열) / DB가 들어가면 굉장히 효율적으로 바꿀 수 있음
...
# csv파일에서 BlockChain을 불러와라
def readBlockchain(blockchainFilePath, mode = 'internal'):
    print("readBlockchain")
    importedBlockchain = []

    try:
        # with as 구문 : 안전장치 / 파일을 열다가 잘못되었을 때 메모리상에 그대로 남아있는 것을 방지
...
def mineNewBlock(difficulty=g_difficulty, blockchainPath=g_bcFileName):
    ...
    while not newBlockFound:
        ...
        # [0:difficulty] 0부터 difficulty까지 (난이도를 맞출 때 까지)
...
# 띄울 때 처음 불리는 함수, 인접 노드를 확인해서 인접 노드의 정보를 확인 (없으면 요청)
def initSvr():
    print("init Server")
    ...
    def do_GET(self):
        ...
        if None != re.search('/block/*', self.path):
            ...
            if None != re.search('/block/getBlockData', self.path):
                ...
                t = threading.Thread(target=mine) # threading.Thread : 공식
                t.start() # Thread가 돈다
...
try:
    ...
    server = ThreadedHTTPServer(('', PORT_NUMBER), myHandler) # 쓰레드 처리(여러개가 동시다발적으로 오면 그 처리)를 가능하게 해줌
